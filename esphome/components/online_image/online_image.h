...
class OnlineImage : public PollingComponent,
 ...
 public:
  ...
  void add_header(const char *key, TemplatableValue<optional<std::string>> value) { this->headers_.insert({key, value}); }
 ...
 protected:
  ...
  std::string url_{""};
  std::map<const char *, TemplatableValue<optional<std::string>>> headers_{};
